...
describe('Polygon creation', () => {
  ...
  it('Should create an element with id g1 and class polygon', async () => {
    ...
    try {
      points = [[ 48, 160], [ 48, 220], [ 263, 220], [ 263, 160]];
      // console_log(points);
      ...
      element = await driver.findElement(By.css('polygon'));
    }
    catch (event) {
      console.log(event);
    }
    finally {
      expect(_class).to.equal('polygon', 'class');
